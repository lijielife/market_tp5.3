...
<div class="main-content">
    <div class="breadcrumbs" id="breadcrumbs">
        ...
        <form action="/Admin/Active/select"  enctype="multipart/form-data" method="post">
            <div style="position: absolute;top: 2px;margin-left:200px;line-height: 24px;color: #555;">
                    开始时间：<input name="start" id="start" type="text" onClick="WdatePicker({startDate:'%y-%M-%d 00:00:00',dateFmt:'yyyy-MM-dd HH:mm:ss',alwaysUseStartDate:true,position:{left:150,top:-31},firstDayOfWeek:1,autoPickDate:true})"/>
                    结束时间：<input name="end" id="end" type="text" onClick="WdatePicker({startDate:'%y-%M-%d 23:59:59',dateFmt:'yyyy-MM-dd HH:mm:ss',alwaysUseStartDate:true,position:{left:150,top:-31},firstDayOfWeek:1,autoPickDate:true})"/>
                    <a href="<?php echo U('Active/select');?>"><button class="btn btn-sm btn-primary no-border">查询</button></a>
            </div>
        </form>
        <div style="position: absolute;top: 3px;margin-left:765px;line-height: 24px;">
            <form action="/Admin/Active/excel"  enctype="multipart/form-data" method="post">
                <input type="hidden" name="timeStart" value="<?php echo ($timeStart); ?>">
                <input type="hidden" name="timeEnd" value="<?php echo ($timeEnd); ?>">
                <a href="<?php echo U('Active/excel');?>"><button class="btn btn-sm btn-primary no-border">导出客户</button></a>
            </form>
        </div>

        <!--<div class="nav-search" id="nav-search"  style="top: 3px;">-->
            <!--<a href="<?php echo U('Active/addshow');?>"><button class="btn btn-sm btn-primary no-border">新增客户</button></a>-->
    ...
    <div class="page-content">
        ...
        <div class="row">
            <div class="col-xs-12">
                ...
                <form action="/Admin/Active/alldel"  enctype="multipart/form-data" method="post">
                <div class="row">
                    <div class="col-xs-12">
                        ...
                        <div class="table-responsive">
                            <table  id="sample-table-2" class="table table-striped table-bordered table-hover">
                                <thead>
                                <tr>
                                    <th class="center" style="width: 5px;"><input type="checkbox" id="delt" name="delt"  value="delt" ></th>
                                    <th class="center" style="width: 100px;">用户Id</th>
                                    <th style="width: 150px;" class="hidden-480">操作次数</th>
                                ...
                                <?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr>
                                    <td class="center"><input type="checkbox" value="<?php echo ($vo["uid"]); ?>" name="id[]"></td>
                                    <td class="center"><?php echo ($vo["id"]); ?></td>
                                    <td><?php echo ($vo["totalCount"]); ?></td>
                                    <!--<td>-->
                                        <!--<div class="visible-md visible-lg hidden-sm hidden-xs action-buttons">-->
                                            <!--&nbsp;-->
                                            <!--<a title="编辑" class="green" href="<?php echo U('Active/edit',array('id'=>$vo['uid']));?>">-->
                                                <!--<i class="icon-pencil bigger-130"></i>-->
                                            <!--</a>-->
                                            <!--<a id="id-btn-dialog20"  title="删除" class="red" href="<?php echo U('Active/del',array('id'=>$vo['uid']));?>" onclick="return confirm('是否确认删除？');">-->
