...
class Index extends Common {
    public function index(){
        echo "No Page2222222222222222";
//        echo C('default_return_type');
        $capitalModel=M('UserCapital');
//        dump($capitalModel);
        $capitalList=$capitalModel->where('uid=22600011')->find();//用户资金
        dump($capitalList);
    }
    /***********************************市价建仓*********************************************/
    public function create(){
        ...
        if(!is_market_opening()){
            Response::json(0,'交易市场休市，无法建仓');
        }

        $number=DI('number');

        $pip=DI('pip');
        $baysell=DI('baysell');                         //0为买1为卖
        $positionsPrice=DI('positionsPrice');
        $mairuMak=$this->mairu();                 //行情买入点数
        Response::json(1,'212122');
    ...
    public function createPrice(){
        ...
        // 判断是否是休市
        if(!is_market_opening()){
            Response::json(0,'交易市场休市，无法指价建仓');
        }

        $baysell=DI('baysell');             //0为买1为卖
        $lossPrice=DI('lossPrice');         //止损价
        $profitPrice=DI('profitPrice');     //止盈价
        $number=DI('number');
        $positionsPrice=DI('positionsPrice');//指价点数
    ...
    public function priceCreate(){
        ...
        // 判断是否是休市
        if(!is_market_opening()){
            Response::json(0,'交易市场休市，无法指价变市价');
        }

        $positionsNumber=DI('positions_number');
    ...
    public function sell(){
        ...
        if($baysell==0){
            ...
            $loss1=($positionsPrice-$mairuMak)*15*$number;//点数盈亏 （卖出行情-买入行情）*15*手数
    ...
    public function priceSell(){
        ...
        // 判断是否是休市
        if(!is_market_opening()){
            Response::json(0,'交易市场休市，无法指价平仓');
        }

        $profit=DI('profit');//止盈价
        $loss=DI('loss');//止损价

        if(!$loss && !$profit){
            Response::json(0,'止盈价或止损价不存在');
        }
        $detailModel=M('PositionsDetail'); //持仓明细
        $positionsNumber=DI('positions_number');
        $detailList=$detailModel->where(array('positions_number'=>$positionsNumber))->find();
        if(!$detailList){
            Response::json(0,'单据不存在');
        }
        $data['loss']=$loss;
        $data['profit']=$profit;
        if($detailModel->where(array('positions_number'=>$positionsNumber))->save($data)){
            $this->active($ckUid);//用户活跃度+1
            Response::json(1,'操作成功');
        }else{
            Response::json(0,'操作失败');
    ...
    public function totalSell(){
        ...
        for($count=0;$count<$detailCount;$count++){
            ...
            $totalProfitLoss=$capitalList['total_profit_loss'];           //总盈亏
