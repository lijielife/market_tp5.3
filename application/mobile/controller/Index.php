...
class Index extends Common {
    public function index(){
        echo 'No Page11222';
        echo "<br/>";
        echo C('config_test')."<br/>";
        $capitalModel=M('UserCapital');
        $capitalList=$capitalModel->select();
//        dump($capitalList);
        echo C('SERVER_NAME');
...
//        header("Content-type: application/json; charset=utf-8");
        ...
        $baysell=$info['baysell'];                   //0为买1为卖
        $positionsPrice=$info['positionsPrice'];
        ...
        if($baysell==0){
            if(abs($positionsPrice-$mairuMak)<=$pip){
                ...
                $usableMargin1=$usableMargin-$charge-$uccupy;  //可用保证金 可用保证金-手续费-占用保证金
                if($usableMargin1<=0){
...
//        header("Content-type: application/json; charset=utf-8");
        ...
        $baysell=$info['baysell'];             //0为买1为卖
        $lossPrice=$info['lossPrice'];         //止损价
        $profitPrice=$info['profitPrice'];     //止盈价
        $number=$info['number'];
        $positionsPrice=$info['positionsPrice'];//指价点数
    ...
    public function priceCreate(){
        ...
        $positionsNumber=$info['positions_number'];
